...
export default function TrendsPage() {
  ...
  const { trends, isLoading, feedError, refresh } = useTrendFeed(platform);

  // Debug logging
  useEffect(() => {
    console.log("TrendsPage loaded. Platform:", platform, "isLoading:", isLoading, "feedError:", feedError);
  }, [platform, isLoading, feedError]);
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
            {filteredTrends.map((trend, index) => (
              <TrendCard key={trend.source_url} trend={trend} index={index} />
            ))}
          </div>
        )}
      </section>
