...
function HeatmapLayer({ hotspots }: { hotspots: HotspotData[] }) {
  const map = useMap();
  const heatmapRef = useRef<any>(null);

  useEffect(() => {
    // Dynamically load leaflet-heat library
    ...
    script.onload = () => {
      ...
      // Convert hotspots to heatmap data points [lat, lng, intensity]
      // Normalize score to 0-1 range for intensity (max score ~1000)
      const heatData = hotspots.map((hotspot) => [
        hotspot.lat,
        hotspot.lng,
        Math.min(1, hotspot.score / 1000),
      ]);
  ...
  return (
    <div className="relative h-full w-full overflow-hidden rounded-[24px]">
      ...
      >
        ...
        {/* Add interactive markers on top */}
