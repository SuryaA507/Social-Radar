...
const SUPPORTED_PLATFORM_VALUES = new Set(["all", "youtube", "reddit"]);
...
export default function MapRegionPanel({ region }: { region: string }) {
  ...
  useEffect(() => {
    ...
    const fetchRegionIntelligence = async () => {
      setIsLoading(true);
      setError(null);

      try {
        if (!SUPPORTED_PLATFORM_VALUES.has(selectedPlatform)) {
          const apiBase = getApiBase();
          const fallbackResponse = await fetch(
            `${apiBase}/api/map/region?${new URLSearchParams({ name: region }).toString()}`,
            { cache: "no-store" },
          );
          if (!fallbackResponse.ok) {
            throw new Error(`Regional intelligence failed with ${fallbackResponse.status}`);
          }
          const fallbackPayload: RegionIntelligenceResponse = await fallbackResponse.json();
          if (active) {
            setData({
              ...fallbackPayload,
              top_trends: [],
              recent_content: [],
              platforms: {
                ...fallbackPayload.platforms,
                [selectedPlatform]: [],
              },
            });
            setError(`${selectedPlatform.toUpperCase()} integration pending`);
          }
          return;
        }
        ...
        const response = await fetch(`${apiBase}/api/map/region?${params.toString()}`, {
          cache: "no-store",
        });
        if (!response.ok) {
          throw new Error(`Regional intelligence failed with ${response.status}`);
        }
        ...
        if (active) {
          ...
          setError(
            fetchError instanceof Error
              ? fetchError.message
              : "Failed to load regional intelligence",
          );
        }
      } finally {
        if (active) {
  ...
  return (
    <aside className="glass-panel h-full rounded-[30px] p-5 sm:p-6">
      <div className="panel-header mb-5 pb-4">
        ...
        <h1 className="mt-2 text-2xl font-semibold text-foreground">{region}</h1>
        <p className="mt-2 text-sm leading-7 text-muted">
          Region-specific narrative analysis across active social channels.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-[24px] border border-card-border/80 bg-black/20 px-4 py-10 text-center text-sm text-muted">
          No intelligence available for this region.
        </div>
      ) : (
        <div className="space-y-5">
          {error ? (
            <div className="rounded-[20px] border border-amber-glow/25 bg-amber-glow/8 px-4 py-3 text-sm text-amber-glow">
              {error}
            </div>
          ) : null}
