...
def _fetch_search_videos(api_key: str, query: str, limit: int) -> list[dict[str, Any]]:
    search_payload = _youtube_get(
        ...
        params={
            ...
            "type": "video",
            "order": "relevance",
            "maxResults": min(max(limit, 1), MAX_RESULTS_PER_REGION),
            "key": api_key,
        },
    )
...
def fetch_youtube_trends(
    *,
    ...
    normalized_query = (search_query or "").strip().lstrip("#")
    ...
    if normalized_query:
        try:
            items = _fetch_search_videos(api_key, normalized_query, requested_limit)
            for item in items:
                try:
                    normalized = _normalize_item(
                        item,
                        region="Global",
                        topics=(normalized_query.lower(),),
                    )
                    if not normalized:
                        continue
                    normalized["keyword"] = f"#{normalized_query}" if search_query and search_query.strip().startswith("#") else normalized_query.title()
                    if normalized["source_url"] in seen_urls:
                        continue
                    seen_urls.add(normalized["source_url"])
                    trends.append(normalized)
                except Exception as exc:
                    logger.debug("[YOUTUBE] Failed to normalize search item: %s", exc)
                    continue
        except (requests.RequestException, YouTubeAPIError) as exc:
            logger.warning("[YOUTUBE] Search failed for %s: %s", normalized_query, exc)
        except Exception as exc:
            logger.error("[YOUTUBE] Unexpected search error for %s: %s", normalized_query, exc)
